...
class User_Manager(models.Manager):
    def user_validator(self, form_data):
        ...
        if form_data['password'] != form_data['confirm_password']:
            errors['password_match'] = "Your password doesn't match! Make sure you typed correctly!"
        # if birthday > thirteen_years_ago:
        #     errors['birthday'] = "COPPA COPPA COPPA"
...
class Job_Manager(models.Manager):
    def job_validator(self, form_data):
        ...
        if len(form_data['description']) < 3:
            errors['description'] = "Bro, that's not enough to describe the job, use at least 3 characters!"
        if len(form_data['location']) < 3:
            errors['location'] = "That is NOT a valid location, dawg. Use at LEAST 3 characters!"
