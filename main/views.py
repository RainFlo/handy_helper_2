...
def process_login(request):
    ...
    if len(errors) > 0:
        ...
        for msg in errors.values():
            messages.error(request, msg)
        return redirect('/')
    # returns a list of users with that same email
    ...
    if bcrypt.checkpw(request.POST['password'].encode(), users_email[0].password.encode()):
        ...
        return redirect('/')
    messages.error(request, "The password that you provided doesn't match the password that we have on file!\nTry again!")
...
def new_job(request):
    context = {
        'logged_in_user': User.objects.get(id=request.session['user_id']),
        # 'all_categories': Real_Category.objects.all()
    }
...
def create_job(request):
    ...
    jobby_job = Job.objects.create(
        title = request.POST['title'],
        description = request.POST['description'],
        location = request.POST['location'],
        user = User.objects.get(id=request.session['user_id'])
    )
    cat_add = Real_Category.objects.create(title=request.POST['real_category'])
    if cat_add != "None":
        jobby_job.real_categories.add(cat_add)

    cat_other = Real_Category.objects.create(title=request.POST['other'])
    if len(cat_other.title) != 0:
        jobby_job.real_categories.add(cat_other)
    return redirect('/dashboard')


def logout(request):
    # del request.session['user_id'] OR
